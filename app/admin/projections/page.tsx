...
import ProjectionList from '../../components/projections/ProjectionsList';
...
const ProjectionsPage: React.FC = () => {
  ...
  const fetchProjections = async (page: number) => {
    try {
      ...
      const response = await fetch(`https://back-k1a3.onrender.com/projections/?page=${page}&is_active=true`, {
        method: 'GET',
        headers: myHeaders,
      });
      if (!response.ok) {
      ...
      setProjections(Array.isArray(data.results) ? data.results : []);
  ...
  const deleteProjection = async (id: number) => {
    try {
      ...
      const response = await fetch(`https://back-k1a3.onrender.com/projections/${id}/`, {
        method: 'DELETE',
        headers: myHeaders,
      });
      if (!response.ok) {
      ...
      setProjections(projections.map(projection => projection.id === id ? { ...projection, state: false } : projection));
    } catch (error) {
  ...
  const handleSort = (column: keyof Projection) => {
    ...
    const sortedProjections = [...projections].sort((a, b) => {
      ...
      // Asegurarse de que estamos comparando valores del mismo tipo
      if (typeof aValue === 'string' && typeof bValue === 'string') {
        return order === 'asc' ? aValue.localeCompare(bValue) : bValue.localeCompare(aValue);
      } else if (typeof aValue === 'number' && typeof bValue === 'number') {
        return order === 'asc' ? aValue - bValue : bValue - aValue;
      }
      return 0; // Si son de tipos diferentes o son undefined, no se ordena
    });
  ...
  const filteredProjections = projections.filter(projection =>
    (projection.movie as any)?.title.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div id="content-container" className="content-container">
          <div id="items-actions" className="projections-actions">
            <div id="new-item-button-container" className="new-item-button-container">
              <Link href="/admin/nuevoprojection">

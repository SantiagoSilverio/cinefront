...
const LoginRegister: React.FC = () => {
  ...
  const toggleForm = () => {
    setIsLogin(!isLogin);
    setErrorMessage("");
    // Clear unnecessary states when toggling forms
    if (isLogin) {
      setName("");
      setPhone("");
      setAddress("");
    ...
    try {
      ...
      if (!response.ok) {
        const message = response.status === 401
          ? "Verifica las credenciales o permisos."
          : data.message || "Error al registrarse";
        Toast.fire({ icon: "error", title: "Error", text: message });
        return;
      }
    ...
    <div className="relative flex h-screen overflow-hidden">
      ...
      <div className={`absolute inset-0 flex transform ${isLogin ? "translate-x-0" : "-translate-x-full"} transition-transform duration-1000`}>
        <div className="w-1/2 flex flex-col justify-center items-center bg-pink-500 p-8">
          ...
          <button className="mt-8 py-2 px-4 bg-green-500 text-white rounded-md" onClick={toggleForm}>Registrarse</button>
        </div>
      </div>
      ...
      <div className={`absolute inset-0 flex transform ${isLogin ? "translate-x-full" : "translate-x-0"} transition-transform duration-1000`}>
        <div className="w-1/2 flex flex-col justify-center items-center bg-pink-600 p-8">
          <h2 className="text-3xl font-bold text-white mb-20">Registrarse</h2>
          <form className="space-y-6 w-full max-w-md" onSubmit={handleRegister}>
            <div className="flex flex-col items-center">
              ...
              <div className="w-full mt-4">
                <label htmlFor="register-city" className="block text-lg font-medium text-white">Ciudad</label>
                <input
                  type="text"
                  id="register-city"
                  value={city}
              ...
              <div className="w-full mt-12 flex justify-center">
                <button type="submit" className="py-2 px-4 bg-green-500 text-white rounded-md">Registrarse</button>
              </div>
            </div>
          </form>
          <button className="mt-8 py-2 px-4 bg-green-500 text-white rounded-md" onClick={toggleForm}>Iniciar sesión</button>
        </div>
      </div>
    </div>
  );
